...
""", encoding="utf-8")
    ...
    def test_art_names_the_volume_the_recipe_already_refuses(self):
        recipe = (game.FRAMEWORK / "recipes/visual.md").read_text(encoding="utf-8")
        self.assertTrue(
            game.recipe_refuses_table_volume(recipe),
            "a receita já recusa que a mesa seja volume",
        )
        self.assertEqual(game.art_rain_volume_source(), "recipes/visual.md")
        starter = Path(game.FRAMEWORK) / "assets/starters/canvas-arcade"
        report = game.art_reading(starter)
        ...
        self.assertIn("(`volume`)", item["scope"])
        ...
        self.assertNotIn("mesa seja volume", report["scope"])
        self.assertNotIn("mesa seja volume", game.art_direction_scope())
    ...
    def test_git_names_the_reading_the_process_already_refuses(self):
        guide = (game.FRAMEWORK / "references/process.md").read_text(encoding="utf-8")
        self.assertTrue(
            game.process_refuses_hash_as_reading(guide),
            "o processo já recusa que o hash seja leitura",
        )
        self.assertEqual(game.git_identity_source(), "references/process.md")
        ...
        self.assertIn("(`leitura`)", report["scope"])
        self.assertNotIn("leitura", report)
        self.assertFalse(game.process_refuses_hash_as_reading(""))
        with mock.patch.object(game, "git_identity_source", return_value=None):
            silent = game.git_summary(game.FRAMEWORK)
        self.assertNotIn("hash seja leitura", silent["scope"])
        recipe = (game.FRAMEWORK / "recipes/create.md").read_text(encoding="utf-8")
        skill = (game.FRAMEWORK / "SKILL.md").read_text(encoding="utf-8")
        readme = (game.FRAMEWORK / "README.md").read_text(encoding="utf-8")
        self.assertIn("nomeia a leitura que o processo já recusa", recipe)
        self.assertIn("nomeia a leitura que o processo já recusa", skill)
        self.assertIn("nomeia a leitura que o processo já recusa", readme)
        self.assertNotIn("verified", report["scope"])
        self.assertNotIn("hash seja leitura", game.verify_scope())
        self.assertNotIn("hash seja leitura", game.next_scope())
    ...
    def test_bar_at_floor_names_the_optimist_the_guide_already_refuses(self):
        guide = (game.FRAMEWORK / "references/production-bar.md").read_text(encoding="utf-8")
        self.assertTrue(
            game.bar_refuses_optimistic_table_as_observation(guide),
            "a barra já recusa que a tabela otimista seja observação",
        )
        self.assertEqual(game.bar_optimist_source(), "references/production-bar.md")
        empty = game.bar_reading(self.project)
        self.assertEqual(empty["at_floor"], [])
        self.assertEqual(game.bar_at_floor_keys(empty), [])
        self.declare_bar({key: ("slice", "shippable") for key in game.BAR_DIMENSIONS} | {"pacing": ("playable", "slice")})
        report = game.bar_reading(self.project)
        item = report["at_floor"]
        self.assertEqual(item["keys"], ["pacing"])
        self.assertEqual(item["keys"], game.bar_at_floor_keys(report))
        self.assertEqual(game.bar_declaration(self.project)["at_floor"], ["pacing"])
        self.assertIn(
            "a tabela otimista seja observação",
            item["scope"],
            "o bar listava o piso e calava a recusa",
        )
        self.assertIn("(`otimista`)", item["scope"])
        self.assertNotIn("otimista", item)
        self.assertFalse(report["assessed"])
        self.assertFalse(game.bar_refuses_optimistic_table_as_observation(""))
        with mock.patch.object(game, "bar_optimist_source", return_value=None):
            silent = game.bar_reading(self.project)
        self.assertNotIn(
            "a tabela otimista seja observação",
            silent["at_floor"]["scope"],
        )
        recipe = (game.FRAMEWORK / "recipes/production.md").read_text(encoding="utf-8")
        skill = (game.FRAMEWORK / "SKILL.md").read_text(encoding="utf-8")
        readme = (game.FRAMEWORK / "README.md").read_text(encoding="utf-8")
        self.assertIn("nomeia a otimista que a barra já recusa", guide)
        self.assertIn("nomeia a otimista que a barra já recusa", recipe)
        self.assertIn("nomeia a otimista que a barra já recusa", skill)
        self.assertIn("nomeia a otimista que a barra já recusa", readme)
        self.assertNotIn("verified", item["scope"])
        self.assertNotIn("aprovado", item["scope"])
        self.assertNotIn("4.5", item["scope"])
        self.assertNotIn("a tabela otimista seja observação", report["scope"])
        if report.get("undeclared"):
            self.assertNotIn(
                "a tabela otimista seja observação",
                report["undeclared"].get("scope") or "",
    ...
    def test_bar_keeps_the_lower_tier_when_two_documents_disagree(self):
        ...
        self.assertEqual(report["at_floor"]["keys"], ["feel"])
        self.assertEqual(game.bar_at_floor_keys(report), ["feel"])
        self.assertEqual(game.bar_declaration(self.project)["at_floor"], ["feel"])
    ...
    def test_origins_declare_fields_name_the_consistency_the_recipe_already_refuses(self):
        ...
        self.assertEqual(reading["fields"]["names"], ["origin", "author", "license"])
        self.assertEqual(game.origins_field_names(reading), ["origin", "author", "license"])
        skill = (game.FRAMEWORK / "SKILL.md").read_text(encoding="utf-8")
        readme = (game.FRAMEWORK / "README.md").read_text(encoding="utf-8")
    ...
    def test_feel_reads_named_constants_and_never_claims_to_have_felt_them(self):
        ...
        self.assertIn("src/game/rules.js", report["sources"]["paths"])
        self.assertIn("data/spawn.json", report["sources"]["paths"])
    ...
    def test_feel_names_the_universal_the_recipe_already_refuses(self):
        recipe = (game.FRAMEWORK / "recipes/feel.md").read_text(encoding="utf-8")
        self.assertTrue(
            game.recipe_refuses_universal_constants(recipe),
            "a receita já recusa que o valor seja constante universal",
        )
        self.assertEqual(game.feel_constant_universal_source(), "recipes/feel.md")
        starter = Path(game.FRAMEWORK) / "assets/starters/canvas-arcade"
        report = game.feel_reading(starter)
        ...
        self.assertIn("(`universais`)", item["scope"])
        self.assertNotIn("universais", item)
        self.assertFalse(report["felt"])
        ...
        self.assertIn("nomeia o universal que a receita já recusa", skill)
        self.assertIn("nomeia o universal que a receita já recusa", readme)
        self.assertNotIn("valor seja constante universal", report["scope"])
        self.assertNotIn("valor seja constante universal", game.observation_item_scope())
    ...
    def test_feel_sources_names_the_capture_the_recipe_already_refuses(self):
        recipe = (game.FRAMEWORK / "recipes/feel.md").read_text(encoding="utf-8")
        self.assertTrue(
            game.recipe_refuses_disk_capture_as_session(recipe),
            "a receita já recusa que captura no disco seja sessão observada",
        )
        self.assertEqual(game.feel_capture_source(), "recipes/feel.md")
        starter = Path(game.FRAMEWORK) / "assets/starters/canvas-arcade"
        self.assertIn("src/game/rules.js", game.feel_source_files(starter))
        report = game.feel_reading(starter)
        item = report["sources"]
        self.assertIn("src/game/rules.js", item["paths"])
        self.assertEqual(item["paths"], game.feel_source_files(starter))
        self.assertIn(
            "captura no disco seja sessão observada",
            item["scope"],
            "o feel listava o fonte e calava a recusa",
        )
        self.assertIn("(`captura`)", item["scope"])
        self.assertNotIn("captura", item)
        self.assertFalse(report["felt"])
        self.assertFalse(game.recipe_refuses_disk_capture_as_session(""))
        empty = game.feel_reading(self.project)
        self.assertEqual(empty["sources"], [])
        with mock.patch.object(game, "feel_capture_source", return_value=None):
            silent = game.feel_reading(starter)
        self.assertNotIn(
            "captura no disco seja sessão observada",
            silent["sources"]["scope"],
        ...
        skill = (game.FRAMEWORK / "SKILL.md").read_text(encoding="utf-8")
        readme = (game.FRAMEWORK / "README.md").read_text(encoding="utf-8")
        self.assertIn("nomeia a captura que a receita já recusa", recipe)
        self.assertIn("nomeia a captura que a receita já recusa", skill)
        self.assertIn("nomeia a captura que a receita já recusa", readme)
        self.assertNotIn("verified", item["scope"])
        self.assertNotIn("aprovado", item["scope"])
        self.assertNotIn("4.5", item["scope"])
        self.assertNotIn("captura no disco seja sessão observada", report["scope"])
        if report.get("then"):
            self.assertNotIn(
                "captura no disco seja sessão observada",
                report["then"].get("scope") or "",
