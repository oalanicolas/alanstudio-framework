...
`shipped` é sempre falso. Sem o passo, `next` propõe `ship.unpacked`. Pacote sem passo não é o empacote. Se a receita recusa que o pacote sem passo seja o empacote, o `unpacked` do `ship` nomeia o empacote que a receita já recusa. Manifesto no disco não é outra máquina. Sem chave `empacote`.
