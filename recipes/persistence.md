...
banco, não de um ajuste de estrutura. Se a receita recusa que o ajuste de estrutura seja a migração, o `save` nomeia o ajuste que a receita já recusa. Formato no disco não é o ajuste. Sem chave `ajuste`.

Separe as três categorias antes de escrever qualquer coisa no disco:

- **Efêmero:** existe só durante a partida e é reconstruído ao iniciar. Se a receita recusa que o efêmero seja o save, o `save` nomeia o efêmero que a receita já recusa. Partida no disco não é o efêmero. Sem chave `efêmero`. Se a receita recusa que o reconstruído seja o save, o `save` nomeia o reconstruído que a receita já recusa. Início no disco não é o reconstruído. Sem chave `reconstruído`.
- **Persistente de progresso:** o que o jogador conquistou e não aceitaria perder. Se a receita recusa que a conquista seja o save, o `save` nomeia a conquista que a receita já recusa. Rótulo no disco não é a conquista. Sem chave `conquista`.
- **Persistente de preferência:** volume, controles, acessibilidade, idioma. Vive
  separado do progresso, porque sobrevive a apagar a partida. Se a receita recusa que apagar a partida apague o idioma, o `save` nomeia o idioma que a receita já recusa. Partida no disco não é o idioma. Sem chave `idioma`. Se a receita recusa que apagar a partida apague os controles, o `save` nomeia os controles que a receita já recusa. Partida no disco não é os controles. Sem chave `controles`. Se a receita recusa que apagar a partida apague a acessibilidade, o `save` nomeia a acessibilidade que a receita já recusa. Partida no disco não é a acessibilidade. Sem chave `acessibilidade`. Se a receita recusa que apagar a partida apague o volume, o `save` nomeia o volume que a receita já recusa. Partida no disco não é o volume. Sem chave `volume`. Se a receita recusa que o derivado prove a inconsistência, o `save` nomeia a inconsistência que a receita já recusa. Derivado no disco não é a inconsistência. Sem chave `inconsistência`. Se a receita recusa que o derivado prove os caminhos, o `save` nomeia os caminhos que a receita já recusa. Derivado no disco não é os caminhos. Sem chave `caminhos`. Se a receita recusa que a suspensão do dispositivo prove a interrupção, o `save` nomeia a suspensão que a receita já recusa. Suspensão no disco não é a interrupção. Sem chave `suspensão`. Se a receita recusa que a bateria prove a interrupção, o `save` nomeia a bateria que a receita já recusa. Bateria no disco não é a interrupção. Sem chave `bateria`. Se a receita recusa que o processo encerrado prove a interrupção, o `save` nomeia o processo que a receita já recusa. Processo no disco não é a interrupção. Sem chave `processo`. Se a receita recusa que a perda de foco prove a interrupção, o `save` nomeia o foco que a receita já recusa. Foco no disco não é a interrupção. Sem chave `foco`. Se a receita recusa que a aba fechada prove a interrupção, o `save` nomeia a fechada que a receita já recusa. Fechada no disco não é a interrupção. Sem chave `fechada`. Se a receita recusa que o arquivo temporário prove a substituição, o `save` nomeia o temporário que a receita já recusa. Temporário no disco não é a substituição. Sem chave `temporário`.
...
Trate dado inválido como caso normal, não como exceção: Se a receita recusa que dado inválido seja exceção, o `save` nomeia a exceção que a receita já recusa. Dado no disco não é a exceção. Sem chave `exceção`. arquivo truncado, campo
...
interrupção no meio não deixe um save pela metade. Se a receita recusa que a interrupção no meio deixe um save pela metade, o `save` nomeia a metade que a receita já recusa. Meio no disco não é o save. Sem chave `metade`. Defina os momentos de gravação
...
preferência. Se a receita recusa que o flush grave esses eixos, o `save` nomeia os eixos que a receita já recusa. Flush no disco não é o save. Sem chave `eixos`. Se a receita recusa que o convite seja preferência, o `invite` do `playtest` nomeia a preferência que a receita já recusa. Convite no disco não é a sessão. Sem chave `preferência`. Escolher no painel grava. Query no disco não é
...
em curso não. Se a receita recusa que a outra aba vista o progresso em curso, o `save` nomeia o curso que a receita já recusa. Preferência no disco não é o curso. Sem chave `curso`. Ouvir não é aba fechada. Se a receita recusa que ouvir seja aba fechada, o `used` do `save` nomeia a audição que a receita já recusa. Ouvir no disco não é a aba. Sem chave `audição`. Isto **não** é aba fechada observada e não sobe `state_trust`. Se a receita recusa que isto seja aba fechada observada, o `save` nomeia a observada que a receita já recusa. Save no disco não é a aba observada. Sem chave `observada`. Grava
