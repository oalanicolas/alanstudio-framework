...
| Laboratório com jogos (o caso normal) | `discover --root <lab>` lê cada jogo e devolve o que os distingue, inclusive os sinais que o `next` usa (primeiro ciclo, ofício, feel sem recibo, achado sem forma, convite e origem sem recibo); a ordem é a do disco — **não trate a primeira linha como prioridade**. Sinal verdadeiro não é partida jogada. Lista de arquivo sem recibo não é licença |
