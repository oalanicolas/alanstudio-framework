...
7. **Produzir até o acabamento, pela dimensão mais baixa.** Quando o recorte já
   ...
   **`art` / `content` / `ship`** leem paleta ou art-bible vigente, mesas
   ...
   `next` nomeia `ship.artifact_open`. Nomear não executa. Se a receita recusa que nomear o comando execute, o `artifact_open` do `ship` nomeia a execução que a receita já recusa. Comando no disco não é outra máquina. Sem chave `execução`. Se a receita recusa que compartilhar o convite seja elsewhere, o `release` do `ship` nomeia o compartilhar que a receita já recusa. Arquivo no disco não é outra máquina. Sem chave `compartilhar`. Se a receita recusa que a receita autorize publicar, o `release_current` do `ship` nomeia o autoriza que a receita já recusa. Arquivo no disco não é outra máquina. Sem chave `autoriza`. `elsewhere`
   continua falso.
