...
| O verbo funciona mas não convence | `feel`; se `unobserved`, `note --author … --note "o que o verbo sentiu"`. Sem caminho, o único jogo do laboratório basta. Lê rumble e as janelas da chuva; nomear não é `felt`. Depois `roles` e `context --focus audio` |
