...
| O verbo funciona mas não convence | `feel`; se `unobserved`, `note --author … --note "o que o verbo sentiu"`. Sem caminho, o único jogo do laboratório basta. Lê rumble, o peso do passo e as janelas da chuva; nomear não é `felt`. Depois `roles` e `context --focus audio` |
...
7. **Produzir até o acabamento, pela dimensão mais baixa.** Quando o recorte já
   ...
   **`feel [<projeto>]`** lê constantes de perdão/hitstop/câmera no `CONFIG`,
   o peso do passo (`player.speed`, o avanço) e
   as janelas da chuva (`practiceTicks`, `recoveryTicks`, o fecho) — o
   campo já as marca; o comando calava o passo. Lê o
