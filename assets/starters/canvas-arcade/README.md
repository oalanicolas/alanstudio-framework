...
| `accessibility` | `slice` | `shippable`: contraste verificado por medição em cena no dispositivo — `npm run contrast` amostra pixels do stub após `draw()`; o aviso e o overlay nomeiam as teclas do remapeamento vigente; `docs/access.md` declara o que o recorte não atende; sessão com uma mão ainda não foi observada |
...
tools/        servidor, orçamento, mix, sessão, tamanho, export, nascer mesa, nascer look e gerar sfx
...
tokens não é volume, direção consistente nem alguém de fora no piso. Os papéis do verbo e a cama em `public/sfx` entram no mixer;
