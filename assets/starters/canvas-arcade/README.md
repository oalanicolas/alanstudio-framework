...
| `accessibility` | `slice` | `shippable`: contraste verificado por medição em cena no dispositivo — `npm run contrast` amostra pixels do stub após `draw()`; o aviso, o overlay e o `cycle.hand` nomeiam IJKL + P/O; `cycle.touch` e `cycle.pad` nomeiam toque e controle e o stub percorre o verbo nesses mapas; `docs/access.md` declara o que o recorte não atende; sessão com uma mão ainda não foi observada |
