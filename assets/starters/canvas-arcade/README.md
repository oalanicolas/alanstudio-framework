...
espelha continuar e reiniciar. Na porta e no fim ela nomeia a
...
npm run session    # partida simulada → totais e curva; `--spawn` escolhe a chuva; `session --look` e `session --speed` nomeiam o que o convite já lê; não sobrescreve `played` sem `--force`; não é sessão observada
...
| --- | --- | --- |
| `feel` | `playable` | `slice`: cada ação com sinal próprio de partida, contato e término — dash, coleta, guarda e dano já têm squash/hitstop/tremor/câmera, rumble e rastro distintos; o avanço estreita dois ticks antes de alongar (`squashCoil`) e o coil marca o rumo no corpo sem inventar faixa no HUD; a guarda que já tem corrente senta os mesmos dois ticks (`squashBankCoil`) e também atravessa o estilhaço — o quadro da conversão também atravessa; o coil do avanço não some quando a guarda pede no mesmo tick — a guarda espera o coil e o land; o sit não come a pose do dash; o pedido não decai no travel; o arco da guarda não mente que o dash está pronto — a faixa trava como no `bankLock`; a coleta no avanço não congela nem senta o dash — parada continua com hitstop e sit; o raspo no avanço não come o alongamento; coleta e guarda no mesmo quadro continuam na hora; orbe que cai no arco da guarda espera — o sit não inflama a aposta; depois do commit o orbe entra; o relógio não come a guarda que já sentou — o sit converte antes do `over`; o hitstop no fim não alonga o relógio — o limite encerra mesmo durante o congelamento; o dash agora aterrissa (senta, câmera, puff, tap e voz próprios) e o quadro do land também atravessa — a recuperação depois continua vulnerável; a graça pós-dano também vale no mesmo quadro — o segundo estilhaço raspa, não empilha impacto; dois orbes no mesmo quadro não inflam a corrente — o segundo espera o próximo tick; orbe e estilhaço no mesmo quadro: o estilhaço letal resolve e o orbe espera — a ordem do array não decide a aposta; a porta fecha o mesmo arco (`dash`+`land`) sem contar o ofício e sem recovery; a corrente mora no corpo em pips, não só no HUD; a coleta leva o orbe ao slot, o erro espalha, a guarda deposita e o fim derruba a aposta não guardada — o corpo senta (`squashOver`), o quadro senta (tremor, flash e punch do último verbo não atravessam o overlay); na pausa o quadro senta do mesmo jeito e o corpo fica na pose congelada, larga a pose de jogo e o overlay nomeia o que caiu e o recorde que o HUD já mostrava; no fim a cortina do over vence a pausa — P e aba escondida não comem a aposta — recorde zero some — e a queda vence a cortina, que reusa a placa do look; as legendas nascem depois da cortina; o raspo risca o campo, estreita o corpo quando o verbo está parado — no avanço e no land o alongamento e o sit permanecem —, empurra a câmera na direção e acende menos que a queda, sem hitstop e sem pulso; a ameaça marca o trilho e a câmera inclina para o mesmo aviso, a recuperação do dash veste o apoio — ainda vulnerável, não a tinta da prática; o corpo aponta para o último avanço; a porta recebe o movimento e o aviso pede mover antes do avanço — a fantasia do `--idea` não come essa janela: depois da frase a porta ainda ensina a abrir; depois da porta o campo não repete a frase nem o mover — dash e coleta entram; o hold leva o mesmo relógio — retomar não devolve o ensino; hold antigo sem o número não inventa ensino feito; sem porta a frase continua o primeiro aviso; a mostra toca o corpo — acende e estreita sem pontuar — o trilho marca a mostra e o live nomeia o perigo na porta; a porta fala `live` uma vez quando a mostra começa, sem ligar a cama; a porta senta e fala sem contar o ofício e o erro acende o campo; o orbe que cai acende o campo, marca o lugar, desloca a câmera para baixo e senta o corpo quando o verbo está parado — no avanço, no coil e no sit da guarda a queda marca o chão e não senta o compromisso; sem pulsar o controle; o aviso nomeia a queda enquanto a corrente é zero; a prática contorna o campo na tinta do orbe e some quando a ameaça começa; o aviso nomeia essa janela (`hint_practice`) — dash, hit, miss e a superfície vencem; depois da prática o aviso pede o orbe; a porta não ensina; a guarda contorna o campo na tinta da corrente enquanto a chuva folga; o fecho contorna o campo, pulsa o controle a cada segundo, o dado `closeIntervalScale` aperta o intervalo e `closeHazardScale` sobe o estilhaço, sem faixa no HUD; com corrente viva o aviso pede guardar de novo — pad e toque continuam calados depois da primeira guarda; `npm run probe` conta os buffers, não o peso percebido; escrever no recado não dispara o verbo; a escuta do remap come a tecla; o botão focado não dispara o verbo; o toque que sai do campo ainda solta; a aba escondida não deixa o verbo preso; a perda de foco não deixa o ofício pendente; o controle que some não deixa a partida correr; o harness não jogou |
...
| `art_direction` | `slice` | `shippable`: um implementador que não participou da direção produz o próximo item dentro do piso, e a comparação em movimento confirma — a receita está no art-bible; a paleta é mesa e `dusk` e `calm` são o segundo e o terceiro look (`?look=` / `settings.look`); a página também veste esses tokens; os knobs (select, faixa, foco) também; halo e vinheta dão volume ao recorte geométrico; o corpo aponta, o orbe é círculo e o estilhaço é losango; a porta chove a mesa vigente sem comer a seed; `look --from` / `--as` nasce o próximo — estilhaço e perigo permanecem; o art-bible nomeia perigo e as janelas do campo; três looks não aprovam direção; `consistent` é falso |
| `audio_mix` | `slice` | `shippable`: faixa dinâmica controlada, sem clipping que obrigue a baixar o volume — o palco tem folga, a cama ocupa o barramento de música e o mixer limita o master; coleta e guarda sobem de tom com a corrente e a legenda nomeia essa aposta; o erro nomeia a corrente que `lost` derrubou e cala quando era zero — não herda `chain`; coleta, queda, raspo, impacto, avanço, o término, a guarda e o fim levam o x do campo; o término fala no barramento de sfx; o orbe perdido fala no barramento de sfx; o fecho, a prática e a guarda falam no barramento de UI com legenda; o pulso do fecho refresca a linha que já está lá — dez "últimos segundos" não comem collect/bank/hit; a voz continua a cada segundo; no fecho a cama sobe o tom com o pulso (`closeBedRate`) em cima do relógio da sessão; no `over` a cama solta com fade (`BED_FADE_MS`); pause, title e aba escondida continuam cortando a cama seco; no campo a pausa `hush` corta as vozes do verbo; no fim e na porta o `hush` não corre — o stinger atravessa; `sfx --from` / `--as` nasce a próxima voz no papel que o mixer já toca; `npm run mix` soma cama e vozes na simulação com a mesma taxa, não no dispositivo; loudness percebido não foi medido; `heard` é falso |
| `pacing` | `slice` | `shippable`: a curva foi observada com quem nunca viu o jogo — a prática é orbe-só, o campo a marca e o aviso a nomeia; guardar recupera o intervalo e o campo marca a folga; o fecho aperta o intervalo e sobe o risco a partir do dado; a sessão relata never_banked e erro repetido na simulação; `/?invite=1` some a tabela de comandos; com last-run, `/?invite=1&seed=<n>` abre essa partida e `&spawn=<mesa>` / `&look=<paleta>` / `&speed=<relógio>` juntam chuva, look e relógio se o candidato os nomeou; depois do fim a página mostra seed, pontos, eixos e a curva que o last-run já traçou e oferece os quatro nomes para copiar ou gravar — o achado copiado e gravado leva essa faixa; número na faixa não preenche os quatro; copiar não grava; gravado não fecha a curva; a curva com quem nunca viu o jogo continua pendente |
| `state_trust` | `slice` | `shippable`: interrupção abrupta real — aba fechada — além do teste de dado inválido; `pagehide`, `beforeunload` e perda de foco já descarregam o save; o blur da janela grava o hold e senta o relógio no campo e no fim; o controle que some senta o mesmo relógio se a sessão falou no pad; na porta a aba escondida, o blur e o pad que some só descarregam — não congelam a mostra; schema 3 guarda `hold` (o tick interrompido e o relógio da porta) e `canResume` o lê — Continuar continua sendo repetir `lastSeed`; retomar não devolve o ensino; hold antigo sem `attractTick` não inventa ensino feito; a abertura lê `lastSeed`, a última pontuação e o recorde; a porta e o fim nomeiam sessão volátil e gravação que não ficou; preferências ilegíveis avisam no painel, no live e no canvas da porta e do fim e preservam `settings.broken`; a outra aba veste as preferências desta página; stub, `pagehide` e `beforeunload` no teste não são aba fechada real; `trusted` é falso |
| `performance` | `playable` | `slice`: orçamento de quadro declarado e cena representativa medida nele; cada tick compacta a chuva no mesmo array e reusa o poço; evento, telegraph, rastro e o gerador da chuva também reusam; `npm run budget` cronometra as cenas `title.attract` e `playing.run` (mostra da porta e partida + `draw` num canvas stub) e relata o reuso — não o compositor nem o dispositivo alvo |
| `accessibility` | `slice` | `shippable`: contraste amostrado no stub após `draw()` — `npm run contrast` relata o par sem limiar; o dispositivo alvo ainda não foi observado; o aviso do primeiro ciclo nomeia teclado, toque, controle e a prática orbe-só; o dash e o mapa da superfície que falou também ganham passo no campo; na porta e no fim o canvas nomeia a lacuna do som que o painel já mostra; o overlay confirma o mapa da superfície que falou por último e segue `uiScale`; a casca da página também segue o mesmo knob; o `cycle.hand` nomeia IJKL + P/O; a página remapeia as seis ações do teclado e a tabela nomeia as teclas vigentes; a escuta come a tecla que escolhe o verbo; o botão focado não dispara o verbo; `gameSpeed` dilata o relógio da partida (não a queda da chuva, nem a mostra da porta, nem o fim) e a cama segue esse relógio só na partida; `assist` cede queda e alcance também na mostra da porta; `reducedMotion` na porta trava canvas, toque e aviso vivo na mesma chuva parada; `colorblind` não esmaga a chuva do look que já separa quente e frio — o par azul/laranja do padrão é o fallback quando as duas tintas ainda compartilham o eixo; o campo do look permanece; na porta a região viva nomeia o toque da mostra (`a mostra toca` / `a mostra raspa`) sem fingir coleta; a região viva nomeia o aviso do primeiro ciclo que o canvas já pinta — o convite some a tabela; no fim a linha some; a região viva nomeia a pausa (`pausado`) só quando o overlay diz Pausado — no fim e na porta a palavra some; no fim, na porta e na pausa no campo, o placar e o recorde que o canvas já mostra — no fim também a corrente que caiu se o overlay a nomeia; na porta e no fim também o aviso da sessão se o canvas o mostra — jogando sem pausa o número não entra; o overlay da pausa reusa o placar (`Pausado — N`) e o recorde se houver, como o fim; `docs/access.md` declara o que o recorte não atende; o pulso no aparelho (dash, land, collect, bank, hit, over e o fecho) some com `reducedMotion` e pausa — queda, raspo e mostra não pulsam; o pad que some senta o relógio no disco se a sessão falou no pad — teclado não; stub não é sessão no controle; sessão com uma mão ainda não foi observada |
| `content_scale` | `shippable` | `flagship`: a ferramenta é boa o bastante para alguém de fora produzir no piso — `dusk` e `calm` são a segunda e a terceira chuva e o jogo as consome por `?spawn=` / `settings.spawnProfile`; `table --from` de spawn, dusk ou calm com `--as denser` nasce chuva distinta; `--as brief` encurta prática e rampa sem comer a folga da guarda; `pair --from` nasce look e chuva no mesmo nome e `?mood=` aplica; `session --spawn` traça essa chuva; `session --look` e `session --speed` nomeiam o convite; três chuvas e um comando não são volume; `enough` é falso |
...
uma linha com contagem. O pulso do fecho refresca a linha que já está
lá: dez "últimos segundos" não empurram o verbo para fora da faixa.
A voz do fecho continua a cada segundo. No centro inferior, onde nascia, ela caía sobre
...
vence o look. `colorblind` não esmaga a chuva que já separa quente e
frio; o par do padrão é o fallback. O harness lê os três; consistência
em movimento continua pendente. Chuva e texto do HUD passam por `src/game/tables.js`.
...
parte da prática longa; `--as brief` encurta prática e rampa e deixa a
folga da guarda; sem `--as` a cópia é idêntica nos knobs.
