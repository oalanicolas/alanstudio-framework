...
| `accessibility` | `slice` | `shippable`: contraste verificado por medição em cena no dispositivo — `npm run contrast` amostra pixels do stub após `draw()`; o aviso, o overlay e o `cycle.hand` nomeiam IJKL + P/O; o `cycle.touch` nomeia o mapa de toque e o stub percorre mover/avançar/guardar; `docs/access.md` declara o que o recorte não atende; sessão com uma mão ainda não foi observada |
