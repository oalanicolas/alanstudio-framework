...
| --- | --- | --- |
| `feel` | `playable` | `slice`: cada ação com sinal próprio de partida, contato e término — dash, coleta, guarda e dano já têm squash/hitstop/tremor/câmera, rumble e rastro distintos; o dash agora aterrissa (senta, câmera, puff e tap próprios); a corrente mora no corpo em pips, não só no HUD, e no erro os pips quebram para fora; a ameaça marca o trilho, a recuperação do dash muda a silhueta e o erro acende o campo; `npm run probe` conta os buffers, não o peso percebido; o harness não jogou |
