...
- Legendas para toda informação sonora (`captions`); a faixa vence a cortina da pausa e do fim no stub; coleta e guarda nomeiam a corrente que o tom já sobe — isso não é sessão observada
...
- Região viva (`#live`): fase, pausa, perigo, última legenda e, no fim e na porta, o placar e o recorde que o canvas já mostra; no fim, a corrente que caiu se o overlay a nomeia; na porta o toque da mostra (`a mostra toca` / `a mostra raspa`) sem fingir coleta. Texto no DOM não é sessão observada
