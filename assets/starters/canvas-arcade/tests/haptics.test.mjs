...
test("cada verbo pulsa com duração e magnitude distintas", () => {
  ...
  for (const role of ["dash", "collect", "bank", "hit", "over"]) {
  ...
  assert.equal(new Set(durations).size, 5, "duração igual apaga o peso do verbo");
  assert.equal(new Set(weaks).size, 5, "magnitude igual apaga o peso do verbo");
  assert.equal(new Set(strongs).size, 5);
