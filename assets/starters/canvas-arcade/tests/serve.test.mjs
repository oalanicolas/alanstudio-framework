...
for (const name of ["farol", "Farol do Sul"]) {
  test(`o servidor entrega o jogo a partir de "${name}"`, async () => {
    ...
    try {
      ...
      assert.doesNotMatch(JSON.stringify(saved), /aprovado|verified|LUFS|4\.5\s*:\s*1/);
      ...
      assert.doesNotMatch(JSON.stringify(record), /aprovado|verified|LUFS|4\.5\s*:\s*1/);
      ...
      assert.doesNotMatch(finding, /aprovado|verified|LUFS|4\.5\s*:\s*1|outsider/);
      ...
      assert.doesNotMatch(JSON.stringify(attached), /aprovado|verified|LUFS|4\.5\s*:\s*1/);
...
test("o banner do serve nomeia o relógio que o jogo já lê", () => {
  ...
  assert.doesNotMatch(banner, /aprovado|verified|LUFS|4\.5\s*:\s*1/);
});
