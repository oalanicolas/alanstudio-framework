...
test("o export copia o jogo e deixa de fora o que só serve para desenvolver", async () => {
  ...
  try {
    ...
    assert.ok(existsSync(join(dist, "public/sfx/land.wav")), "o artefato leva o tap do término");
    ...
    assert.ok(existsSync(join(dist, "public/sfx/stir.wav")), "o artefato leva o tap da guarda");
    assert.ok(existsSync(join(dist, "public/sfx/missed.wav")), "o artefato leva o tap do orbe perdido");
    ...
    try {
      ...
      const noted = await fetch(`http://localhost:${port}/playtest/note`, {
        method: "POST",
        headers: { "content-type": "application/json" },
