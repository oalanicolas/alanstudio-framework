...
`play` e `open` são o mesmo verbo. Com tela, o avanço abre a porta. Depois de uma partida, a página grava o recibo se você escrever; o próximo comando do harness continua `note`. `executed` fica `false`. Não cria pasta e não serve. A frase de agora sai em stderr; encanar o stdout continua o recibo.
