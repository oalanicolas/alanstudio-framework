...
uiScale, preset de uma mão (o ciclo nomeia IJKL + P/O quando o starter declara `hand`), assistência e velocidade da partida. `verified` é sempre `false`. Trocar no stub não é sessão observada. `save` procura
...
`?seed=<n>` abre essa partida e ignora o hold. O `next` aponta
