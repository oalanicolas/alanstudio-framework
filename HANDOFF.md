...
**HEAD:** ver `git log -1` — vigente 0.9.451: o `craft` nomeia a definição que a pesquisa já recusa. Pesquisa no disco não é ofício observado. Sem chave `definição`. Nomear não observa.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**HEAD anterior + confirmação pendente:** suítes do 0.9.450 (446 / 509). Confirmação 0.9.451 depois das suítes.
...
## O que o HEAD já entrega (0.9.91–0.9.451)
...
`craft` nomeia a definição que
a pesquisa já recusa. A
pesquisa já dizia que número
sem definição declarada não é
critério; o problema copiava o
achado e calava a recusa.
Pesquisa no disco não é
ofício observado. Sem chave
`definição`. Nomear não
observa. Anexar o `scope` só
em `craft_reading`, não em
`craft_declaration`.
