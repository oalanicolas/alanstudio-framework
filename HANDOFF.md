...
as teclas, o cluster de uma mão (`hand`), o toque, o controle e as queries de look e chuva. `next` só entra em `then.lost`. Depois de um `init` fresco,
...
`contrast` é alcance, não look; `look --from` / `--as` nasce o próximo),
