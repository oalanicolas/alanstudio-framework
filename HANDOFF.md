...
**HEAD:** ver `git log -1` — vigente 0.9.388: o `sfx verify` nomeia a integridade que o check já cruza. Hash no disco não é mix ouvida. Sem chave `sha256`. Não promove `heard`.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes confirmadas no HEAD (0.9.388):** `python3 -m unittest discover -s tests` → 384 OK.
