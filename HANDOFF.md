...
**HEAD:** ver `git log -1` — vigente 0.9.453: o `sources[n]` do continuity nomeia a fila que o processo já recusa. Fonte no disco não é backlog. Sem chave `fila`. Nomear não executa.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes confirmadas no HEAD (0.9.453):** `python3 -m unittest discover -s tests` → 449 OK.
...
## O que o HEAD já entrega (0.9.91–0.9.453)
...
`sources[n]` do continuity nomeia
a fila que o processo já
recusa. O processo já dizia
que `sources_found` não comprova
fila atual; a fonte copiava o
caminho e calava a recusa.
Fonte no disco não é backlog.
Sem chave `fila`. Nomear não
executa.
