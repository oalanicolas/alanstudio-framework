...
**HEAD:** ver `git log -1` — vigente 0.9.464: o `sfx import` nomeia a improvisação que a receita já recusa. Importar no disco não é licença. Sem chave `improvisar`. Nomear não ouve.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes confirmadas no HEAD (0.9.464):** `python3 -m unittest discover -s tests` → 463 OK.
...
## O que o HEAD já entrega (0.9.91–0.9.464)
...
`sfx import`
nomeia a improvisação que a
receita já recusa. A receita já
dizia que tocar na página não
autoriza improvisar licença; o
import copiava a conta e calava a
recusa. Importar no disco não é
licença. Sem chave `improvisar`.
Nomear não ouve.
