...
**HEAD:** ver `git log -1` — vigente 0.9.408: o `record` nomeia a medição que o roteiro já recusa. Recibo no disco não é observação. Sem chave `mede`. Nomear não observa.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**HEAD anterior + confirmação pendente:** `python3 -m unittest discover -s tests` → 403 OK no 0.9.407.
`cd assets/starters/canvas-arcade && npm test` → 509 OK. Esperado no 0.9.408: 404 / 509.
...
## O que o HEAD já entrega (0.9.91–0.9.408)
