...
**HEAD:** ver `git log -1` — vigente 0.9.506: o `sfx verify` vazio nomeia a lacuna que a receita já recusa. Lista no disco não é mix. Sem chave `lacuna`. Nomear não ouve.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes:** confirmadas no HEAD (0.9.506): 567 / 516.
...
## O que o HEAD já entrega (0.9.91–0.9.506)
...
2. **Idéia→jogo:** o `-h` lista `start` antes de `init`. `start` devolve `open` e `url`; `play` / `open`
   ...
   Se o check cruza a integridade, o `sfx verify` nomeia a integridade que o check já cruza. Hash no disco não é mix ouvida. Sem chave `sha256`. Se a receita recusa que variante ausente seja lacuna, o `sfx verify` vazio nomeia a lacuna que a receita já recusa. Lista no disco não é mix. Sem chave `lacuna`.
