...
**HEAD:** ver `git log -1` — vigente 0.9.459: o `candidate_tally` nomeia o cinco que a pesquisa já recusa. Conta no disco não é sessão observada. Sem chave `cinco`. Nomear não observa.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes confirmadas no HEAD (0.9.459):** `python3 -m unittest discover -s tests` → 455 OK.
...
## O que o HEAD já entrega (0.9.91–0.9.459)
...
`candidate_tally`
nomeia o cinco que a
pesquisa já recusa. A pesquisa já
dizia que cinco playtesters não é
critério; a conta copiava os verbos
e calava a recusa. Conta no disco
não é sessão observada. Sem chave
`cinco`. Nomear não observa.
