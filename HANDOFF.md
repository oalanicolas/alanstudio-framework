...
## O que o HEAD já entrega (0.9.91–0.9.463)
...
`conflicts[n]`
do `bar` nomeia a precedência que a
barra já recusa. A barra já dizia
que duas linhas discordantes não se
resolvem por precedência; o
conflito copiava as fontes e calava
a recusa. Linha no disco não é
acabamento. Sem chave `precedência`.
Nomear não observa.
