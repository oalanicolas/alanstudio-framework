...
**HEAD:** ver `git log -1` — vigente 0.9.564: o `sources` do `roles` nomeia a imediata que a receita já recusa. Sinal no disco não é o sistema. Sem chave `imediata`. Nomear não ouve.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes:** confirmadas no HEAD (0.9.564): **625 / 516**.
...
## O que o HEAD já entrega (0.9.91–0.9.564)
...
| content_scale | shippable | o `content` nomeia o codificado que a receita já recusa; o then do `guide` nomeia a estrutura que a receita já recusa; o then do `init` nomeia o scaffold que a receita já recusa; o `fields` do `origins --declare` nomeia a consistência que a receita já recusa; o `gate` scale nomeia a repeatability que a receita já recusa; o `content` nomeia a composição que a receita já recusa; o `content` nomeia a migração que as mesas já compartilham; o `start` nomeia o par que o pair já nasce; o `content` nomeia a mesa que o disco já nasce; o `content` nomeia o par que `listMoods` já lista; `brief` não come a folga da guarda; paleta não finge volume; dusk+calm+pair; `enough` falso |
