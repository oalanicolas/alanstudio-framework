...
**HEAD:** ver `git log -1` — vigente 0.9.504: o `sfx seed` nomeia a aprovação que a receita já recusa. Seed no disco não é mix. Sem chave `aprovação`. Nomear não ouve.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes:** confirmadas no HEAD (0.9.504): 565 / 516.
...
## O que o HEAD já entrega (0.9.91–0.9.504)
...
2. **Idéia→jogo:** o `-h` lista `start` antes de `init`. `start` devolve `open` e `url`; `play` / `open`
   ...
   `sfx search` nomeia o stem do starter que casa. Se o `tools/design-sfx.*` desloca a voz, o `sfx search` nomeia o deslocamento que o sfx já oferece. Arquivo no disco não é mix ouvida. Sem chave `sfx`. Se a receita recusa que o acervo compartilhado seja o primeiro ciclo, o `local` do `sfx search` nomeia o adapt que a receita já recusa. Stem no disco não é mix. Sem chave `adapt`. `sfx info`
