...
**HEAD:** ver `git log -1` — vigente 0.9.427: o `next` nomeia a criação que o roteiro já recusa. Proposta no disco não é pasta criada. Sem chave `criação`. Nomear não cria.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes confirmadas no HEAD (0.9.426):** `python3 -m unittest discover -s tests` → 422 OK.
`cd assets/starters/canvas-arcade && npm test` → 509 OK.
Confirmação pendente no 0.9.427.
...
## O que o HEAD já entrega (0.9.91–0.9.427)
