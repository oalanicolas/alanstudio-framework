...
## O que o HEAD já entrega (0.9.91–0.9.357)
...
`feel` nomeia o perdão que o
probe já exercita. O tool já
contava o buffer; o comando
calava. Conta no disco não é
peso percebido. Sem chave
`probe`. Não promove `felt`.
Não adicionar `then.probe`.
Não importar limiar de
buffer.
