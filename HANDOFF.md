...
## O que o HEAD já entrega (0.9.91–0.9.421)
...
`scan` nomeia a inexistência
que o roteiro já recusa. O
roteiro já dizia que local
não percorrido não equivale a
conteúdo inexistente; a
cobertura contava documentos e
calava a recusa. Contagem no
disco não é inventário. Sem
chave `inexistente`. Nomear
não observa.
