...
**HEAD:** ver `git log -1` — vigente 0.9.751: o item do craft nomeia o padrão que a pesquisa já recusa. Valor no disco não é o padrão. Sem chave `padrão`. Nomear não observa.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes:** confirmadas no HEAD (0.9.751): **812 / 516**.
...
## O que o HEAD já entrega (0.9.91–0.9.751)
...
| content_scale | shippable | o `content` nomeia os bancos que a receita já recusa; o `content` nomeia a hospedagem que a receita já recusa; o `content` nomeia a resolução que a receita já recusa; o `content` nomeia o alerta que a receita já recusa; o `content` nomeia o material que a receita já recusa; o `coverage` nomeia o lexical que o teach já recusa; o `issues_truncated` da coverage nomeia a falta que o roteiro já recusa; o `truncated` do `origins` nomeia a varredura que o roteiro já recusa; o `inline` do `content` nomeia o código que a receita já recusa; o `embedded` do `origins` nomeia a nova que a receita já recusa; o `external` do `content` nomeia o baixado que a receita já recusa; o `missing` do `origins` nomeia o devolve que o roteiro já recusa; o `fields` do `origins` nomeia os três que o roteiro já recusa; o `form` do `origins` nomeia o crédito que o molde já recusa; o `undeclared` do `origins` nomeia a desconhecida que a guia já recusa; o `files` do `content` nomeia os dados que a receita já recusa; o `receipts` do `origins` nomeia a válida que o roteiro já recusa; o `receipts` do `budget` nomeia a otimização que a receita já recusa; o `content` nomeia o codificado que a receita já recusa; o then do `guide` nomeia a estrutura que a receita já recusa; o then do `init` nomeia o scaffold que a receita já recusa; o `fields` do `origins --declare` nomeia a consistência que a receita já recusa; o `gate` scale nomeia a repeatability que a receita já recusa; o `content` nomeia a composição que a receita já recusa; o `content` nomeia a migração que as mesas já compartilham; o `start` nomeia o par que o pair já nasce; o `content` nomeia a mesa que o disco já nasce; o `content` nomeia o par que `listMoods` já lista; `brief` não come a folga da guarda; paleta não finge volume; dusk+calm+pair; `enough` falso |
