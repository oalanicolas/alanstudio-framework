...
**HEAD:** ver `git log -1` — vigente 0.9.406: o `context` nomeia o navegador que o pacote já recusa provar. Pacote no disco não é comportamento no aparelho. Sem chave `navegador`. Nomear não observa.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**HEAD anterior + confirmação pendente:** `python3 -m unittest discover -s tests` → 401 OK no 0.9.405.
`cd assets/starters/canvas-arcade && npm test` → 509 OK. Esperado no 0.9.406: 402 / 509.
...
## O que o HEAD já entrega (0.9.91–0.9.406)
