...
**HEAD:** ver `git log -1` — vigente 0.9.431: o `bar` nomeia os prazos que a barra já recusa. Linha no disco não é calendário. Sem chave `prazos`. Nomear não observa.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes confirmadas no HEAD (0.9.430):** `python3 -m unittest discover -s tests` → 426 OK.
`cd assets/starters/canvas-arcade && npm test` → 509 OK.
Confirmação pendente no 0.9.431.
...
## O que o HEAD já entrega (0.9.91–0.9.431)
