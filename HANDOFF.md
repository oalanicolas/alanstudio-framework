...
**HEAD:** ver `git log -1` — vigente 0.9.455: o `issues[n]` da coverage nomeia o acidente que o mapa já recusa. Recorte no disco não é falha. Sem chave `acidente`. Nomear não inventaria.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**Suítes confirmadas no HEAD (0.9.455):** `python3 -m unittest discover -s tests` → 451 OK.
...
## O que o HEAD já entrega (0.9.91–0.9.455)
...
`issues[n]` da coverage nomeia
o acidente que o mapa já
recusa. O mapa já dizia que
cobertura desigual não é
acidente; o issue copiava o
motivo e calava a recusa.
Recorte no disco não é falha.
Sem chave `acidente`. Nomear
não inventaria.
