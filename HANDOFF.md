...
**HEAD:** ver `git log -1` — vigente 0.9.409: o `check-plan` nomeia o mérito que o processo já recusa. Forma no disco não é adequação. Sem chave `mérito`. Nomear não valida.
**Goal:** ativo. Não marcar complete. AAA fácil ainda não está provado.

**HEAD anterior + confirmação pendente:** `python3 -m unittest discover -s tests` → 404 OK no 0.9.408.
`cd assets/starters/canvas-arcade && npm test` → 509 OK. Esperado no 0.9.409: 405 / 509.
...
## O que o HEAD já entrega (0.9.91–0.9.409)
