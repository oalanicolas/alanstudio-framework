# Manual operacional da Game Dev

Este manual preserva os contratos minuciosos do harness e suas distinções de
evidência. Ele é leitura sob demanda: a entrada, a navegação e o piso de execução
ficam em `SKILL.md`, `routing.md` e `craft-floor.md`.
...
[workspace ligado](workspace-binding.md), a entrada local é
...
1. **Contexto.** Resolva projeto e tarefa; execute `context <projeto> --focus <foco>`.
   ...
   [auditoria de projeto](project-audit.md), sem pedir um segundo
...
2. **Intenção e prontidão.** Defina fantasia, verbo central, plataforma, cenário,
   ...
   ([ambição](ambition.md)). Leia [processo](process.md)
   — com tela, a primeira superfície é a porta; `play` aponta sem executar —
   e [qualidade](quality.md). Para criação ou pré-produção, siga
   [o ciclo criativo](preproduction.md): Game Brief, MDA/GDD, PoC, PRD/TDD,
...
6. **Comparar, registrar, continuar.** Compare antes/depois em condições equivalentes
   ...
   [o guia](aaa-checklist.md) e grave no canônico — completar linhas não
...
7. **Produzir até o acabamento, pela dimensão mais baixa.** Quando o recorte já
   ...
   ([barra de acabamento](production-bar.md)). Declare em tabela, uma linha
   ...
   **A barra descreve, o gate recusa.** [Os dez gates](gates.md) formalizam
   ...
   **`roles <projeto>`** lê os papéis de áudio que o código declara, o
   ...
   Arquivo no disco não é mix ouvida. Sem chave `sfx`. Se o `tools/wav.*` lê o PCM, o `roles` nomeia o PCM que o wav já lê. Bytes no disco não são mix ouvida. Sem chave `wav`. Se a receita recusa que áudio AAA seja quantidade de arquivos, o `roles` nomeia a quantidade que a receita já recusa. Lista no disco não é mix. Sem chave `quantidade`. `--fill` sugere o acervo ou a ficha
   ...
   **`feel [<projeto>]`** lê constantes de perdão/hitstop/câmera no `CONFIG`,
   ...
   Conta no disco não é peso percebido. Sem chave `probe`. Se o laço senta a guarda, o `feel` nomeia o sit que a guarda já senta. Pose no disco não é peso percebido. Sem chave `bank`. Se o laço emite o término, o `feel` nomeia o land que o dash já emite. Pose no disco não é peso percebido. Sem chave `land`. Se a receita recusa que o autor sugerido seja quem jogou, o `feel` nomeia o autor que a receita já recusa. Recibo no disco não é sessão. Sem chave `autor`. Se a receita recusa que o valor seja constante universal, o `feel` nomeia o universal que a receita já recusa. Número no disco não é lei. Sem chave `universais`. Traço no disco não é peso percebido. Lê o
...
Todo trabalho de jogo acontece numa das três escalas de [ambição](ambition.md).
...
- **O degrau percebido é o mínimo entre as dimensões, não a média.** Procure a mais
  baixa antes de melhorar a que já está alta ([barra](production-bar.md)).
  A barra descreve; o [gate](gates.md) recusa; nenhum comando promove.
...
- **Uma próxima ação, com prompt pronto.** Toda entrega com sequência termina com um
  passo, motivo, prova e o [prompt de continuidade](gauntlet.md) em
...
- **Memória nos lugares certos.** Decisões, provas e preferências no canônico do
  jogo; regra transferível no framework ([aprendizados](learning.md)).
...
Faça a [revisão de entrega](delivery.md): pedido → aceite → artefato →
...
conhecer o harness. Processo detalhado: [processo](process.md),
[qualidade](quality.md), [ciclo criativo](preproduction.md).
Fontes sob demanda: [mapa dos estudos](sources.md). Comandos do harness,
