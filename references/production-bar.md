...
Não é uma nota. Se a barra recusa que o piso seja uma nota, o `floor` do `bar` nomeia a nota que a barra já recusa. Linha no disco não é acabamento. Sem chave `nota`. Não é um selo. Se a barra recusa que a declaração seja um selo, o `sources` do `bar` nomeia o selo que a barra já recusa. Linha no disco não é acabamento. Sem chave `selo`. Nenhum comando promove um jogo a um degrau desta
