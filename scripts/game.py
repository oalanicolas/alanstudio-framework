...
# dados brutos provem a cadeia.
# Sem isto o item do craft
# listava o checklist e calava
# a recusa. Brutos no disco
# não é a cadeia.
CRAFT_RAW = re.compile(r"dados brutos")


def research_refuses_raw_data_as_proving_chain(text):
    return bool(text and CRAFT_RAW.search(text))


def craft_item_raw_source():
    path = FRAMEWORK / "references/observable-criteria-research.md"
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if research_refuses_raw_data_as_proving_chain(text):
        return "references/observable-criteria-research.md"
    return None


def craft_item_raw_scope():
    if not craft_item_raw_source():
        return None
    return (
        " O disco recusa que os dados brutos provem a cadeia "
        "(`brutos`). Brutos no disco não é a cadeia."
    )
...
# A pesquisa já recusa que o
# jogo prove a cadeia. Sem
# isto o item do craft listava
# o checklist e calava a
# recusa. Jogo no disco não é
# a cadeia.
CRAFT_GAME = re.compile(r"jogo \+ captura")


def research_refuses_game_as_proving_chain(text):
    return bool(text and CRAFT_GAME.search(text))


def craft_item_game_source():
    path = FRAMEWORK / "references/observable-criteria-research.md"
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if research_refuses_game_as_proving_chain(text):
        return "references/observable-criteria-research.md"
    return None


def craft_item_game_scope():
    if not craft_item_game_source():
        return None
    return (
        " O disco recusa que o jogo prove a cadeia "
        "(`jogo`). Jogo no disco não é a cadeia."
    )
...
# A receita já recusa que a
# informação crítica prove o
# estado. Sem isto o item
# copiava a nota e calava a
# recusa. Crítica no disco
# não é o estado.
A11Y_CRIT = re.compile(r"informação crítica")


def recipe_refuses_critical_as_proving_state(text):
    return bool(text and A11Y_CRIT.search(text))


def observation_critical_source():
    path = FRAMEWORK / "recipes/accessibility.md"
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_critical_as_proving_state(text):
        return "recipes/accessibility.md"
    return None


def observation_critical_scope():
    if not observation_critical_source():
        return None
    return (
...
# A receita já recusa que o
# tremor prove o estado.
# Sem isto o item copiava a
# nota e calava a recusa.
# Tremor no disco não é
# o estado.
A11Y_TREMOR = re.compile(r"tremor")


def recipe_refuses_tremor_as_proving_state(text):
    return bool(text and A11Y_TREMOR.search(text))


def observation_tremor_source():
    path = FRAMEWORK / "recipes/accessibility.md"
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_tremor_as_proving_state(text):
        return "recipes/accessibility.md"
    return None


def observation_tremor_scope():
    if not observation_tremor_source():
        return None
    return (
...
# A receita já recusa que a
# agressiva prove o estado.
# Sem isto o item copiava a
# nota e calava a recusa.
# Agressiva no disco não é
# o estado.
A11Y_HARSH = re.compile(r"agressiva")


def recipe_refuses_agressiva_as_proving_state(text):
    return bool(text and A11Y_HARSH.search(text))


def observation_agressiva_source():
    path = FRAMEWORK / "recipes/accessibility.md"
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_agressiva_as_proving_state(text):
        return "recipes/accessibility.md"
    return None


def observation_agressiva_scope():
    if not observation_agressiva_source():
        return None
    return (
...
# A receita já recusa que o
# novo impulso prove o evento.
# Sem isto o feel listava o
# recibo e calava a recusa.
# Impulso no disco não é o
# evento.
FEEL_IMPULSE = re.compile(r"novo impulso")


def recipe_refuses_new_impulse_as_proving_event(text):
    return bool(text and FEEL_IMPULSE.search(text))


def feel_observations_impulse_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_new_impulse_as_proving_event(text):
        return "recipes/feel.md"
    return None


def feel_observations_impulse_scope():
    if not feel_observations_impulse_source():
        return None
    return (
        " O disco recusa que o novo impulso prove o evento "
        "(`impulso`). Impulso no disco não é o evento."
    )
...
# A receita já recusa que o
# giro completo prove os
# saltos. Sem isto o feel
# listava o recibo e calava
# a recusa. Giro no disco
# não é os saltos.
FEEL_TURN = re.compile(r"giro completo")


def recipe_refuses_full_turn_as_proving_jumps(text):
    return bool(text and FEEL_TURN.search(text))


def feel_observations_turn_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_full_turn_as_proving_jumps(text):
        return "recipes/feel.md"
    return None


def feel_observations_turn_scope():
    if not feel_observations_turn_source():
        return None
    return (
        " O disco recusa que o giro completo prove os saltos "
        "(`giro`). Giro no disco não é os saltos."
    )
...
# prove a previsão. Sem isto
# o feel listava o recibo e
# calava a recusa.
# Confirmação no disco não é
# a previsão.
FEEL_CONFIRM = re.compile(r"confirmação do impacto")


def recipe_refuses_confirmation_as_proving_prediction(text):
    return bool(text and FEEL_CONFIRM.search(text))


def feel_observations_confirm_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_confirmation_as_proving_prediction(text):
        return "recipes/feel.md"
    return None


def feel_observations_confirm_scope():
    if not feel_observations_confirm_source():
        return None
    return (
        " O disco recusa que a confirmação do impacto prove a previsão "
        "(`confirmação`). Confirmação no disco não é a previsão."
    )
...
# A receita já recusa que
# todos os saltos provem o
# feel. Sem isto o feel
# listava o recibo e calava
# a recusa. Saltos no disco
# não é o feel.
FEEL_HOPS = re.compile(r"todos os saltos")


def recipe_refuses_saltos_as_proving_feel(text):
    return bool(text and FEEL_HOPS.search(text))


def feel_observations_saltos_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_saltos_as_proving_feel(text):
        return "recipes/feel.md"
    return None


def feel_observations_saltos_scope():
    if not feel_observations_saltos_source():
        return None
    return (
        " O disco recusa que todos os saltos provem o feel "
        "(`saltos`). Saltos no disco não é o feel."
    )
...
# A receita já recusa que a
# reação prove o feel. Sem
# isto o feel listava o
# recibo e calava a recusa.
# Reação no disco não é o
# feel.
FEEL_REACT = re.compile(r"reação")


def recipe_refuses_reacao_as_proving_feel(text):
    return bool(text and FEEL_REACT.search(text))


def feel_observations_reacao_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_reacao_as_proving_feel(text):
        return "recipes/feel.md"
    return None


def feel_observations_reacao_scope():
    if not feel_observations_reacao_source():
        return None
    return (
        " O disco recusa que a reação prove o feel "
        "(`reação`). Reação no disco não é o feel."
    )
...
# A receita já recusa que o
# demonstram prove o feel.
# Sem isto o feel listava o
# recibo e calava a recusa.
# Demonstram no disco não é
# o feel.
FEEL_SHOW = re.compile(r"demonstram")


def recipe_refuses_demonstram_as_proving_feel(text):
    return bool(text and FEEL_SHOW.search(text))


def feel_observations_demonstram_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_demonstram_as_proving_feel(text):
        return "recipes/feel.md"
    return None


def feel_observations_demonstram_scope():
    if not feel_observations_demonstram_source():
        return None
    return (
        " O disco recusa que o demonstram prove o feel "
        "(`demonstram`). Demonstram no disco não é o feel."
    )
...
def feel_observations_scope():
    ...
    if labs:
        scope += labs
    jolt = feel_observations_reacao_scope()
    if jolt:
        scope += jolt
...
# A receita já recusa que a
# proveniência prove a instância.
# Sem isto o feel lia o CONFIG
# e calava a recusa.
# Proveniência no disco não é
# a instância.
FEEL_PROVENANCE = re.compile(r"proveniência")


def recipe_refuses_provenance_as_proving_instance(text):
    return bool(text and FEEL_PROVENANCE.search(text))


def feel_provenance_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_provenance_as_proving_instance(text):
        return "recipes/feel.md"
    return None


def feel_provenance_scope():
    if not feel_provenance_source():
        return None
    return (
...
# A receita já recusa que o
# acoplamento estranho prove
# o feel. Sem isto o feel
# lia o CONFIG e calava a
# recusa. Acoplamento no
# disco não é o feel.
FEEL_COUPLE = re.compile(r"acoplamento\s+estranho")


def recipe_refuses_coupling_as_proving_feel(text):
    return bool(text and FEEL_COUPLE.search(text))


def feel_coupling_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_coupling_as_proving_feel(text):
        return "recipes/feel.md"
    return None


def feel_coupling_scope():
    if not feel_coupling_source():
        return None
    return (
        " O disco recusa que o acoplamento estranho prove o feel "
        "(`acoplamento`). Acoplamento no disco não é o feel."
    )



# A receita já recusa que a
# outra curva prove o feel.
# Sem isto o feel lia o
# CONFIG e calava a recusa.
# Curva no disco não é o
# feel.
FEEL_CURVE = re.compile(r"outra curva")


def recipe_refuses_curve_as_proving_feel(text):
    return bool(text and FEEL_CURVE.search(text))


def feel_curve_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_curve_as_proving_feel(text):
        return "recipes/feel.md"
    return None


def feel_curve_scope():
    if not feel_curve_source():
        return None
    return (
...
# A receita já recusa que os
# parâmetros provem o feel.
# Sem isto o feel lia o
# CONFIG e calava a recusa.
# Parâmetros no disco não é
# o feel.
FEEL_PARM = re.compile(r"parâmetros")


def recipe_refuses_params_as_proving_feel(text):
    return bool(text and FEEL_PARM.search(text))


def feel_params_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_params_as_proving_feel(text):
        return "recipes/feel.md"
    return None


def feel_params_scope():
    if not feel_params_source():
        return None
    return (
...
# A receita já recusa que o
# consome prove o feel.
# Sem isto o feel lia o
# CONFIG e calava a recusa.
# Consome no disco não é
# o feel.
FEEL_FEED = re.compile(r"consome")


def recipe_refuses_consome_as_proving_feel(text):
    return bool(text and FEEL_FEED.search(text))


def feel_consome_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_consome_as_proving_feel(text):
        return "recipes/feel.md"
    return None


def feel_consome_scope():
    if not feel_consome_source():
        return None
    return (
        " O disco recusa que o consome prove o feel "
        "(`consome`). Consome no disco não é o feel."
    )


# A receita já recusa que a
# lacuna prove o feel. Sem
# isto o feel lia o CONFIG e
# calava a recusa. Lacuna no
# disco não é o feel.
FEEL_GAP = re.compile(r"lacuna")


def recipe_refuses_lacuna_as_proving_feel(text):
    return bool(text and FEEL_GAP.search(text))


def feel_lacuna_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_lacuna_as_proving_feel(text):
        return "recipes/feel.md"
    return None


def feel_lacuna_scope():
    if not feel_lacuna_source():
        return None
    return (
...
# A receita já recusa que isto
# seja aba fechada observada.
# Sem isto o save lia o schema
# e calava a recusa. Save no
# disco não é a aba observada.
PERSIST_OBSERVED = re.compile(r"Isto \*\*não\*\* é aba fechada observada")


def recipe_refuses_save_as_observed_closed_tab(text):
    return bool(text and PERSIST_OBSERVED.search(text))


def save_observed_source():
    path = PERSIST_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_save_as_observed_closed_tab(text):
        return "recipes/persistence.md"
    return None


def save_observed_scope():
    if not save_observed_source():
        return None
    return (
        " O disco recusa que isto seja aba fechada observada "
        "(`observada`). Save no disco não é a aba observada."
    )
...
# A receita já recusa que os
# saves antigos provem o
# save. Sem isto o save lia o
# schema e calava a recusa.
# Antigos no disco não é o
# save.
PERSIST_OLD = re.compile(r"saves antigos")


def recipe_refuses_oldsaves_as_proving_save(text):
    return bool(text and PERSIST_OLD.search(text))


def save_oldsaves_source():
    path = PERSIST_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_oldsaves_as_proving_save(text):
        return "recipes/persistence.md"
    return None


def save_oldsaves_scope():
    if not save_oldsaves_source():
        return None
    return (
        " O disco recusa que os saves antigos provem o save "
        "(`antigos`). Antigos no disco não é o save."
    )
...
# A receita já recusa que os
# itens provem o save. Sem
# isto o save lia o schema e
# calava a recusa. Itens no
# disco não é o save.
PERSIST_ITEM = re.compile(r"itens")


def recipe_refuses_items_as_proving_save(text):
    return bool(text and PERSIST_ITEM.search(text))


def save_items_source():
    path = PERSIST_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_items_as_proving_save(text):
        return "recipes/persistence.md"
    return None


def save_items_scope():
    if not save_items_source():
        return None
    return (
        " O disco recusa que os itens provem o save "
        "(`itens`). Itens no disco não é o save."
    )
...
# A receita já recusa que as
# faces laterais provem as
# emendas. Sem isto o content
# listava arquivos e calava a
# recusa. Faces no disco não
# é as emendas.
CONTENT_FACES = re.compile(r"faces laterais")


def recipe_refuses_side_faces_as_proving_seams(text):
    return bool(text and CONTENT_FACES.search(text))


def content_faces_source():
    path = CONTENT_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_side_faces_as_proving_seams(text):
        return "recipes/content.md"
    return None


def content_faces_scope():
    if not content_faces_source():
        return None
    return (
        " O disco recusa que as faces laterais provem as emendas "
        "(`faces`). Faces no disco não é as emendas."
    )
...
# content listava arquivos e
# calava a recusa. Superfície
# no disco não é as
# dimensões.
CONTENT_SURFACE = re.compile(r"superfície e a geometria")


def recipe_refuses_surface_as_proving_dimensions(text):
    return bool(text and CONTENT_SURFACE.search(text))


def content_surface_source():
    path = CONTENT_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_surface_as_proving_dimensions(text):
        return "recipes/content.md"
    return None


def content_surface_scope():
    if not content_surface_source():
        return None
    return (
        " O disco recusa que a superfície prove as dimensões "
        "(`superfície`). Superfície no disco não é as dimensões."
    )
...
# A receita já recusa que as
# portas provem as dimensões.
# Sem isto o content listava
# arquivos e calava a recusa.
# Portas no disco não é as
# dimensões.
CONTENT_DOOR = re.compile(r"portas")


def recipe_refuses_doors_as_proving_dimensions(text):
    return bool(text and CONTENT_DOOR.search(text))


def content_doors_source():
    path = CONTENT_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_doors_as_proving_dimensions(text):
        return "recipes/content.md"
    return None


def content_doors_scope():
    if not content_doors_source():
        return None
    return (
        " O disco recusa que as portas provem as dimensões "
        "(`portas`). Portas no disco não é as dimensões."
    )
...
def cycle_scope():
    ...
    if bulk:
        scope += bulk
    bone = cycle_bones_scope()
    if bone:
        scope += bone
    gear = cycle_accessories_scope()
    if gear:
        scope += gear
...
# A receita já recusa que anunciar
# o verbo prove o ativo. Sem
# isto o ciclo anunciava o verbo
# e calava a recusa. Verbo no
# disco não é o ativo.
FEEL_ACTIVE = re.compile(r"startup, ativo")


def recipe_refuses_announcing_verb_as_proving_active(text):
    return bool(text and FEEL_ACTIVE.search(text))


def cycle_active_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_announcing_verb_as_proving_active(text):
        return "recipes/feel.md"
    return None


def cycle_active_scope():
    if not cycle_active_source():
        return None
    return (
...
# A receita já recusa que a
# trava residual prove o
# buffer. Sem isto o ciclo
# anunciava o verbo e calava a
# recusa. Trava no disco não
# é o buffer.
FEEL_RESIDUAL = re.compile(r"trava residual")


def recipe_refuses_residual_lock_as_proving_buffer(text):
    return bool(text and FEEL_RESIDUAL.search(text))


def cycle_residual_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_residual_lock_as_proving_buffer(text):
        return "recipes/feel.md"
    return None


def cycle_residual_scope():
    if not cycle_residual_source():
        return None
    return (
...
# anunciava o verbo e calava a
# recusa. Dash-attack no disco
# não é a prioridade.
FEEL_DASH_ATTACK = re.compile(r"dash-attack")


def recipe_refuses_dash_attack_as_proving_priority(text):
    return bool(text and FEEL_DASH_ATTACK.search(text))


def cycle_dash_attack_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_dash_attack_as_proving_priority(text):
        return "recipes/feel.md"
    return None


def cycle_dash_attack_scope():
    if not cycle_dash_attack_source():
        return None
    return (
...
# A receita já recusa que o
# esticar ossos prove o
# teste. Sem isto o ciclo
# anunciava o verbo e calava
# a recusa. Ossos no disco
# não é o teste.
FEEL_BONES = re.compile(r"esticar ossos")


def recipe_refuses_bones_as_proving_test(text):
    return bool(text and FEEL_BONES.search(text))


def cycle_bones_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_bones_as_proving_test(text):
        return "recipes/feel.md"
    return None


def cycle_bones_scope():
    if not cycle_bones_source():
        return None
    return (
        " O disco recusa que o esticar ossos prove o teste "
        "(`ossos`). Ossos no disco não é o teste."
    )



# A receita já recusa que o
# excluir acessórios prove o
# teste. Sem isto o ciclo
# anunciava o verbo e calava
# a recusa. Acessórios no
# disco não é o teste.
FEEL_GEAR = re.compile(r"excluir acessórios")


def recipe_refuses_accessories_as_proving_test(text):
    return bool(text and FEEL_GEAR.search(text))


def cycle_accessories_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_accessories_as_proving_test(text):
        return "recipes/feel.md"
    return None


def cycle_accessories_scope():
    if not cycle_accessories_source():
        return None
    return (
        " O disco recusa que o excluir acessórios prove o teste "
        "(`acessórios`). Acessórios no disco não é o teste."
    )



# A receita já recusa que as
# proporções de cada avatar
# provem o teste. Sem isto
# o ciclo anunciava o verbo
# e calava a recusa.
# Proporções no disco não
# é o teste.
FEEL_RATIO = re.compile(r"proporções de cada avatar")


def recipe_refuses_proportions_as_proving_test(text):
    return bool(text and FEEL_RATIO.search(text))


def cycle_proportions_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_proportions_as_proving_test(text):
        return "recipes/feel.md"
    return None


def cycle_proportions_scope():
    if not cycle_proportions_source():
        return None
    return (
...
# A receita já recusa que a
# arma prove o teste. Sem
# isto o ciclo anunciava o
# verbo e calava a recusa.
# Arma no disco não é o
# teste.
FEEL_ARM = re.compile(r"arma")


def recipe_refuses_arma_as_proving_test(text):
    return bool(text and FEEL_ARM.search(text))


def cycle_arma_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_arma_as_proving_test(text):
        return "recipes/feel.md"
    return None


def cycle_arma_scope():
    if not cycle_arma_source():
        return None
    return (
...
# A receita já recusa que o
# cursor prove o teste. Sem
# isto o ciclo anunciava o
# verbo e calava a recusa.
# Cursor no disco não é o
# teste.
FEEL_WAND = re.compile(r"cursor")


def recipe_refuses_cursor_as_proving_test(text):
    return bool(text and FEEL_WAND.search(text))


def cycle_cursor_source():
    path = FEEL_RECIPE
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_cursor_as_proving_test(text):
        return "recipes/feel.md"
    return None


def cycle_cursor_scope():
    if not cycle_cursor_source():
        return None
    return (
        " O disco recusa que o cursor prove o teste "
        "(`cursor`). Cursor no disco não é o teste."
    )
...
# A receita já recusa que copiar o
# mapa conte as submissões. Sem
# isto o record gravava o recibo
# e calava a recusa. Cópia no
# disco não é a submissão.
PERF_SUBMITS = re.compile(r"submissões efetivas")


def recipe_refuses_copying_map_as_counting_submissions(text):
    return bool(text and PERF_SUBMITS.search(text))


def record_submit_source():
    path = FRAMEWORK / "recipes/performance.md"
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_copying_map_as_counting_submissions(text):
        return "recipes/performance.md"
    return None


def record_submit_scope():
    if not record_submit_source():
        return None
    return (
...
# A receita já recusa que os
# timers provem o vazamento.
# Sem isto o record gravava o
# recibo e calava a recusa.
# Timers no disco não é o
# vazamento.
PERF_TIMER = re.compile(r"timers")


def recipe_refuses_timers_as_proving_leak(text):
    return bool(text and PERF_TIMER.search(text))


def record_timers_source():
    path = FRAMEWORK / "recipes/performance.md"
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_timers_as_proving_leak(text):
        return "recipes/performance.md"
    return None


def record_timers_scope():
    if not record_timers_source():
        return None
    return (
...
# vazamento. Sem isto o
# record gravava o recibo e
# calava a recusa. Desmontar
# no disco não é o vazamento.
PERF_TEAR = re.compile(r"desmontar")


def recipe_refuses_desmontar_as_proving_leak(text):
    return bool(text and PERF_TEAR.search(text))


def record_desmontar_source():
    path = FRAMEWORK / "recipes/performance.md"
    if not path.is_file() or path.is_symlink():
        return None
    try:
        ...
        return None
    if recipe_refuses_desmontar_as_proving_leak(text):
        return "recipes/performance.md"
    return None


def record_desmontar_scope():
    if not record_desmontar_source():
        return None
    return (
